from queues_metrics import queues_workload, jobs_statuslog_extended
from datetime import datetime, timedelta
import pandas as pd

def retrospective_collector(frequency='1h'):

    start_date = datetime(2023, 11, 20, 0, 0, 0)
    end_date = datetime(2023, 11, 28, 14, 0, 0) # next --> 2022-12-21 09:00:00
    ...
    for i in list_of_dates[::-1]:
        print(i)
        queues_workload(predefined_date=i, queues='actual')
        # queues_hourly_to_db(metric='queues_hourly', predefined_date = i)
        # queues_workload_weighted_detailed(predefined_date = i)

#jobs_statuslog_extended(predefined_date='2023-11-18 13:00:00')
#retrospective_collector()
#queues_workload(predefined_date='2023-10-20 18:00:00', queues='actual')
...
def collect_queues_for_period():
    ...
    while start_date <= end_date:
        print(start_date)
        queues_workload(predefined_date=datetime.strftime(start_date, "%Y-%m-%d %H:%M:%S"),
                                 hours=4, queues='db')
