...
SELECT tstart,
       ...
       round(running_queued/nullif(running_queued + running_running,0), 2) as fullness_weighted,
       round(running_completed/nullif(running_completed + running_running,0), 2) as performance_weighted,
       round(n_queued/nullif(n_queued+n_completed,0),2) as utilization,
       round(n_queued/nullif(n_queued+n_running,0),2) as fullness,
       round(n_completed/nullif(n_completed+n_running,0),2) as performance,
       avg_waiting_time,
       avg_merging_time,
