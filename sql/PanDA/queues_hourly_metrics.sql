...
SELECT trunc(to_date(:from_date, 'YYYY-MM-DD HH24:MI:SS'), 'HH24') as datetime,
                 ...
                 NVL(sum(running_jobs)+sum(holding_jobs)+sum(merging_jobs)+sum(transferring_jobs),0) as executing_jobs,
                 NVL(sum(running_volume)+sum(holding_volume)+sum(merging_volume)+sum(transferring_volume),0) as executing_volume,
