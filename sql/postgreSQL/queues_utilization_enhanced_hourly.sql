with queues as (
    SELECT date_trunc('hour', timestamp :from_date) as datetime,
           queue,
           cpuconsumptionunit,
           site,
           cloud,
    ...
    WHERE datetime >= date_trunc('hour', timestamp :from_date)
      and datetime < date_trunc('hour', timestamp :from_date) + interval '1 hour'
    group by date_trunc('hour', timestamp :from_date),
             queue,
             cpuconsumptionunit,
             site,
             cloud,
...
FROM queues a
    INNER JOIN jobs_timings jt on (a.queue = jt.queue
                                   and jt.execution_end_tstamp between
                                    a.datetime and (a.datetime + interval '1 hour'))
group by a.datetime,
        a.queue,
           a.cpuconsumptionunit,
           a.site,
